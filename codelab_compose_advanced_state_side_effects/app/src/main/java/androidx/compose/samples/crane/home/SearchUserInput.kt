...
fun ToDestinationUserInput(onToDestinationChanged: (String) -> Unit) {
    ...
    LaunchedEffect(editableUserInputState) {
        // use the snapshotFlow API to convert Compose State<T> objects into a Flow.
        // When the state read inside snapshotFlow mutates, the Flow will emit the new value to the collector.
        // In our case, we convert the state into a flow to use the power of flow operators.
        // With that, we filter when the text is not the hint, and collect the emitted items
